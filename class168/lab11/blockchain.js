...
class BlockChain {
  ...
  isValid() {
    ...
    for(let k in this.blocks){
      // Verify that every block (except this.currentBlock) has a valid proof.
      if(!this.blocks[k].verifyProof()){
        return false;
      }

      //Also verify that every block's hash matches what the hash is **supposed** to be.
      if(this.blocks[k].blockHeight!==0){
        if(check_hash !== this.blocks[k].prevBlockHash){
          return false;
        }
      }
