...
function acceptCoin(coin) {
  ...
  console.log(check_signature);

  //parse coin
  let identity_hash = parseCoin(coin.coinString);
  ...
  for (let i = 0; i < COIN_RIS_LENGTH; i++) {
    ...
    IRS[i]=coin.getRis(isLeft,i);

    //verify
    if(identity_hash[random][i] != utils.hash(IRS[i])){
      console.log("verifying hashes fail!");
      return false;
    }
  }
...
function determineCheater(guid, ris1, ris2) {
  ...
  let check=false;
  for (let i = 0; i < COIN_RIS_LENGTH; i++) {
    check = utils.decryptOTP({key:ris1[i], ciphertext:ris2[i], returnType:"string"});
    if(check=="IDENT:alice"){
