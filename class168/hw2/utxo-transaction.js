...
module.exports = class UtxoTransaction extends Transaction {
  ...
  totalInput(block) {
    ...
    //
  	...
  	for(let k in this.from){
  		total_UTXOs += block.balances.get(this.from[k]);
  	}
  ...
  validSignature() {
    ...
    //
  	...
  	for(let k in this.from){
  		// 1) The address matches the public key.
  		if(!utils.addressMatchesKey(this.from[k],this.pubKey[k])){
  			throw 'address is not matches the public key.';
  		}
  		
  		// 2) There is a signature.
  		if(this.sig[k]===undefined){
  			throw 'no signature.';
  		}
  		
  		// 3) The signature of this.id is valid for the public key.
  		if(!utils.verifySignature(this.pubKey[k],this.id,this.sig[k])){
