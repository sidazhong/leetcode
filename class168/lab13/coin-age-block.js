...
module.exports = class CoinAgeBlock extends Block {
  ...
  updateCoinAgeBalances(prevBlock) {
    ...
    // ***YOUR CODE HERE***
    //
    // First, set the previous block winner's coin age to 0.
    //
    // Next, increase the coin age balances for all clients
    ...
    //
    // The following line wll double the size of this.adjustedTarget:
    //
    //    this.adjustedTarget = this.adjustedTarget.shiftLeft(1);
    
  	// First, set the previous block winner's coin age to 0.
    if(prevBlock!=undefined){
    	if(prevBlock.rewardAddr!==undefined){
	  		prevBlock.coinAgeBalances.set(prevBlock.rewardAddr,0);
	    }
	  	
	  	// Next, increase the coin age balances for all clients by the amount of coins currently held. 
    	prevBlock.balances.forEach(function(v, k) {
	  		prevBlock.coinAgeBalances.set(k,v+prevBlock.coinAgeOf(k));
	  	})
